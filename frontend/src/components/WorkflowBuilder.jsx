...
export default function WorkflowBuilder() {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:8080/yaml_v1', {
      method: 'POST',
    ...
    .then(res => res.json())
    .then(data => {
      if (data.yaml) localStorage.setItem('yamal_current_yaml', data.yaml);
    })
    .catch(() => {});
  }, [nodes, edges]);
  ...
  const onDrop = useCallback((event) => {
    ...
    // Adjust position so it drops relatively where the mouse is
  ...
  return (
    <div className="dark-theme-wrapper" style={{ display: 'flex', width: '100vw', height: '100vh', backgroundColor: '#191919' }}>
      <aside className="sidebar" style={{ width: '260px', background: '#222', padding: '20px', borderRight: '1px solid #333', color: '#E6E6E6', zIndex: 10 }}>
